...
class MaleFIS(object):
  def __init__(self):
    ...
    #catch 
    rule1 = ctrl.Rule(self.catch['Novice'], self.q['Novice'])
    rule2 = ctrl.Rule(self.catch['Intermediate'], self.q['Intermediate'])
    rule3 = ctrl.Rule(self.catch['National'], self.q['National'])
    rule4 = ctrl.Rule(self.catch['International'], self.q['International'])
    #finish
    rule5 = ctrl.Rule(self.finish['Novice'], self.q['Novice'])
    rule6 = ctrl.Rule(self.finish['Intermediate'], self.q['Intermediate'])
    rule7 = ctrl.Rule(self.finish['National'], self.q['National'])
    rule8 = ctrl.Rule(self.finish['International'], self.q['International'])
    #slip
    rule9  = ctrl.Rule(self.slip['Novice'], self.q['Novice'])
    rule10 = ctrl.Rule(self.slip['Intermediate'], self.q['Intermediate'])
    rule11 = ctrl.Rule(self.slip['National'], self.q['National'])
    rule12 = ctrl.Rule(self.slip['International'], self.q['International'])
    #wash
    rule13 = ctrl.Rule(self.wash['Novice'], self.q['Novice'])
    rule14 = ctrl.Rule(self.wash['Intermediate'], self.q['Intermediate'])
    rule15 = ctrl.Rule(self.wash['National'], self.q['National'])
    rule16 = ctrl.Rule(self.wash['International'], self.q['International'])
    #length
    rule17 = ctrl.Rule(self.length['Novice'], self.q['Novice'])
    rule18 = ctrl.Rule(self.length['Intermediate'], self.q['Intermediate'])
    rule19 = ctrl.Rule(self.length['National'], self.q['National'])
    rule20 = ctrl.Rule(self.length['International'], self.q['International'])
  ...
  def EvalStroke(self, catch, finish, slip, wash, length):
    ...
    self.q.view(sim=self.rowingFis)
...
#fis.ViewPlots()
print(fis.EvalStroke(69,46,2,12,111))
